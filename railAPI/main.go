...
import (
	...
	"github.com/ayush723/metrorail_api/dbutils"
...
//ScheduleResource links both trains and atations
...
func (t TrainResource) getTrain(request *restful.Request, response *restful.Response) {
	...
	err := DB.QueryRow("select ID, DRIVER_NAME, OPERATING_STATUS FROM train where id =?", id).Scan(&t.ID,
		&t.DriverName, &t.OperatingStatus)
	if err != nil {
		log.Println(err)
...
func (t TrainResource) createTrain(request *restful.Request, response *restful.Response) {
	...
	// Error hadling is obvious here. So omitting...
